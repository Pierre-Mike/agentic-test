...
**Database Expert**
- Keywords: "schema", "migration", "database", "query", "table", "SQL"
- Locations: References to server/src/db/, database files
- Indicators: Database schema changes, migrations, query optimization
- Examples: "Create migration for X", "Optimize query for Y", "Add database table"

**API Expert**
- Keywords: "endpoint", "route", "API", "HTTP", "server", "handler"
- Locations: References to server/src/, API routes
- Indicators: API endpoint creation, route handlers, server logic
- Examples: "Add endpoint for X", "Create API route for Y", "Update server handler"

**Testing Expert**
- Keywords: "test", "tests", "e2e", "unit test", "integration test"
- Locations: References to e2e/, server/index.test.ts, test files
- Indicators: Test creation, test fixes, testing strategy
- Examples: "Write tests for X", "How do I test Y", "Add e2e test"

**Automation Expert**
- Keywords: "CI", "CD", "workflow", "GitHub Actions", "automation", "pipeline"
- Locations: References to .github/workflows/, scripts/
- Indicators: CI/CD pipeline changes, automation scripts
- Examples: "Add CI workflow for X", "Create automation script", "Update GitHub Actions"

**Indexer Expert**
- Keywords: "AST", "parser", "symbol", "reference", "indexing", "code analysis"
- Locations: References to indexer code, parsing utilities
...
- Examples: "Create new slash command for X", "Add hook for logging", "Create new agent", "Add API endpoint for Y", "Write tests for X", "Create PR for X"
...
- Examples: "How do I structure frontmatter?", "What model for coordinators?", "How do I test Y?"
...
**Domain:** {claude-config|agent-authoring|database|api|testing|automation|indexer|github}
...
- Locations: .claude/agents/, agent file names
- Pattern: Agent creation or configuration tasks

**Database Expert (High Confidence):**
- Keywords: schema, migration, database, query, table, SQL
- Locations: server/src/db/
- Verbs: create, optimize, add, migrate, update
- Objects: migration, schema, query, table
- Pattern: Database schema changes, migrations, query optimization

**API Expert (High Confidence):**
- Keywords: endpoint, route, API, HTTP, server, handler
- Locations: server/src/
- Verbs: add, create, update, implement
- Objects: endpoint, route, handler, API
- Pattern: API endpoint creation, route handlers

**Testing Expert (High Confidence):**
- Keywords: test, e2e, unit test, integration test
- Locations: e2e/, *.test.ts files
- Verbs: write, add, create, fix
- Objects: test, tests, integration test, unit test
- Pattern: Test creation, test fixes, testing strategy

**Automation Expert (High Confidence):**
- Keywords: CI, CD, workflow, GitHub Actions, automation, pipeline, script
- Locations: .github/workflows/, scripts/
- Verbs: add, create, update, configure
- Objects: workflow, pipeline, action, script
- Pattern: CI/CD and automation tasks
...
- Provide options for all available expert domains:
  ...
  - "Testing (tests/e2e/test strategy)"
  - "Automation (CI/CD/workflows/scripts)"
...
This is a Bun + Hono + Vite + React monorepo:
```
...
From CLAUDE.md:
- **Type safety**: Use strong typing, avoid `any`, leverage TypeScript's type system
...
- **No premature optimization**: Make it work first, optimize only when needed and measured
- **Functional over OOP**: Prefer pure functions, immutability, and composition over classes and inheritance

### Package Scripts

- `bun run dev` - Start development servers (Turbo)
...
```bash
/do "Add endpoint for health check"

# /do executes Pattern A with api domain:
# 1. api-plan-agent creates spec
# 2. User approval gate
# 3. api-build-agent implements endpoint
...
```bash
/do "Write e2e tests for the login flow"
...
### Automation - Implementation (Pattern A)

```bash
/do "Add CI workflow for pull requests"

# /do executes Pattern A with automation domain:
# 1. automation-plan-agent creates spec
# 2. User approval gate
# 3. automation-build-agent implements workflow
# 4. automation-improve-agent updates expertise
```
...
**Current State:**
- 8 expert domains active: claude-config, agent-authoring, database, api, testing, automation, indexer, github
